...
function ReintegroFormStepOne({ className }) {
  ...
  const listaAfiliadosFiltrados =
    rolSesion === "Titular"
      ? afiliado?.data?.grupoFamiliar?.filter(
          (familiar) => familiar.rol !== "Cónyuge"
        )
      : afiliado?.data?.grupoFamiliar; // Si quien inició sesión es Titular, Cónyuge no me se muestra en el input de paraAfiliado
  ...
  const fechaDePrestacion = reintegro?.fechaDePrestacion
    ? format(addDays(reintegro?.fechaDePrestacion, 1), "yyyy-MM-dd")
    : "";
  ...
  if (isLoadingAfiliado || isLoadingEspecialidades)
